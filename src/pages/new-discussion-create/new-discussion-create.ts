...
@IonicPage()
@Component({
  selector: 'page-new-discussion-create',
  templateUrl: 'new-discussion-create.html',
})
export class NewDiscussionCreatePage {
  personId: any;
  team_id: any;
  selectedTeam: any;
  chats:any[] = [];
  ...
  gruopsIds: any = [];
  personIds: any = [];
  public addClassName: string = '';
  chatDetailsChangeLower: any = [];
  ...
  checkFilterIsActive(){
    setTimeout(() => {
      if($('page-new-discussion-create .filter_overlay .active').length){
  ...
  getDefaultData(afterSearch){
    this.storage.get('loggedInUserData').then((val) => {
      ...
      let searchVal:any = $("page-new-discussion-create #search").val();
  ...
  viewChat(group_id,chat_person_name,photoPath,char_Person_id){
    const chatInfo = {
      from : 1,
      to : 10,
  ...
  clearsearch(){
    if($("page-new-discussion-create #search").val() == "Search"){
      $("page-new-discussion-create #search").val('');
    }
  }
                  ...
                  this.speechRecognition.startListening()
                    .subscribe(
                      (matches: Array<string>) => {
                        $("page-new-discussion-create #search").blur();
                        $("page-new-discussion-create #search").val(matches[0]).focus();
                      },
  ...
  addGroupName(chatDetails,ev) {
    this.chatDetailsChangeLower = Object.keys(chatDetails).reduce((c, k) => (c[k.toLowerCase()] = chatDetails[k], c), {});
    let sendDiscussionDetails: Array<{id : string , discussionName : string}> = [];
    ...
    if($(ev.target).hasClass('select-card') == false){
      $(ev.target).parents(".select-card").addClass("divDisable");
      $(ev.target).parents(".select-card").find(".card-img span").addClass("active");
    }else{
      $(ev.target).addClass("divDisable");
      $(ev.target).find(".card-img span").addClass("active");
    }
    this.newDiscussionDetails.push(sendDiscussionDetails);
  }

  addPersonName(chatDetails,ev) {
    let sendDiscussionDetails: Array<{id : string , discussionName : string, image : string}> = [];
  ...
  deletedPersonName(personId,gruopsIds,personIds,ev){
  ...
  goToDiscussionSavePage(selectData,gruopsIds,personIds){
  ...
  filterData(){
    ...
    this.selectedTeams = [];
    let selectedTeams = [];
    $('page-new-discussion-create .team.active').each(function() {
      if($(this).prop('id') != 0){
    ...
    this.roles = [];
    let roles = [];
    $('page-new-discussion-create .role.active').each(function() {
      if($(this).prop('id') != 0){
